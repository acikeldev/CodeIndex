...
/// <summary>
/// A fully in-memory, thread-safe <see cref="IFileSystem"/> for deterministic tests. All "disk" state
/// lives in concurrent dictionaries so scanners, parsers, caches, the store, and the watcher's pure
/// logic run without touching a real disk. Writes and moves stamp a last-write time (so the 30-minute
/// stale-cache reclaim check behaves), globs are honored in <see cref="EnumerateFiles"/>, and the
/// backing store is concurrent so the snapshot-swap concurrency tests can hammer it.
/// </summary>
public sealed class InMemoryFileSystem : IFileSystem
{
    ...
    /// overridable so that branch is testable in-memory rather than against the real profile folder.
    /// </summary>
    public string UserProfileRoot { get; set; } = @"C:\Users\Test\AppData\Local";

    // ── test setup helpers (not part of IFileSystem) ────────────────────────────
